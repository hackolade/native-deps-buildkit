import { npmCommand, npxCommand } from './commands.js';
...
export async function runPatchPackage() {
	await exec({
		command: npmCommand,
		parameters: ['run', 'patch','--', '--error-on-war', '--error-on-fail'],
		options: {shell: true},
	});
}
...
export async function installAllPrebuildsProvidedAsNPMDependencies(modulesBuildMetadata) {
	...
	await exec({ command: npmCommand, parameters: ['install', '--force', '--no-save', ...prebuildsToInstall] });
}
