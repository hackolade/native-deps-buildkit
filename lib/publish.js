...
import {installForElectronOrNAPI} from '#lib/install.js';
...
export const pkgTpl = ({ githubOrganizationScope, moduleName, targetPlatform, targetArch, forOpenSSL1 }) => {
	const mainFile = (moduleName === 'desktop-trampoline' && 'desktop-trampoline') || 'prebuild.node';
	const files = moduleName === 'desktop-trampoline' && (os.platform() === 'win32' && ['desktop-trampoline.exe', 'ssh-wrapper.exe'] || ['desktop-trampoline', 'ssh-wrapper']) || ['*.node'];
	const opensslSuffix = forOpenSSL1? '-openssl1': '';

	const {version} = buildMetadata[moduleName];
...
export async function preparePackage({ module, targetPlatform, targetArch, electron }) {
	...
	await rm(path.join(module.baseDir, 'build'), {force:true, recursive: true})

	await installForElectronOrNAPI({
		module,
		targetPlatform,
		targetArch,
		electron,
	...
	const {version} = buildMetadata[module.name];
	await writePkgTpl({
...
export async function prepareParcelWatcherPrebuildsPackages() {
	...
	const parcelWatchPrebuilds = parcelPackages
		.filter(entry => entry.isDirectory() && !entry.name.includes('musl') && entry.name !== 'watcher')
		.map(entry => {
			return {
	...
	const pkgRaw = await readFile(path.join(ROOT_DIR, 'node_modules', '@hackolade','watcher', 'package.json'));
	const pkgWatcher = JSON.parse(pkgRaw);

	const hackoladeScopedPackagesPaths = [];
	for (const { name, dir } of parcelWatchPrebuilds) {
		log('change scope for %o from %o', name, dir);
		...
		pkg.repository= {type: 'git', url: 'https://github.com/hackolade/native-deps-buildkit.git'};
		pkg.version = pkgWatcher.version;
...
export async function normalizeNativeModulesUnderHackolade(nativeModules) {
	...
	for (const { name, baseDir } of nativeModules) {
		...
		log('moving native dependencies for %o under @hackolade scope...', sanitizedName);
		...
		pkg.repository= {type: 'git', url: 'https://github.com/hackolade/native-deps-buildkit.git'};
		const {version} = buildMetadata[sanitizedName];

		pkg.version = version;
		const depsByTarget = {
			[`${pkg.name}-linux-x64`]: version,
			[`${pkg.name}-win32-x64`]: version,
			[`${pkg.name}-darwin-x64`]: version,
			[`${pkg.name}-darwin-arm64`]: version,
		};

		// add openssl1 variant for listed modules that require it
		if(sanitizedName === 'couchbase'){
			log('--> adding dependency to openssl alternative for couchbase...');
			depsByTarget[`${pkg.name}-linux-x64-openssl1`] = version;
		}
		log('--> using dependencies for module %O: %O', pkg.name, depsByTarget);
		const deps = pkg.dependencies;
		const newDeps = Object.assign( {}, depsByTarget, deps);
		
		pkg.dependencies = newDeps

		log('--> new package.json for module %O: %O', pkg.name,  pkg);

		const packageAsString = JSON.stringify(pkg)
		log('--> writing new package.json for module %O: %O', pkg.name,  pkg);
		await writeFile(path.join(hackoladeScopedPkgPath, 'package.json'), packageAsString);
		await writeFile(path.join(hackoladeScopedPkgPath, '.npmrc'), ghPackageNpmrc);
		hackoladeScopedPackagesPaths.push(hackoladeScopedPkgPath);

		log('--> module %O moved', pkg.name);
	}
...
export async function checkPackageVersionExistsFromPath({packagePath, token}){
	log('--> use package.json file at %o to check if a version already exists on Github Packages for a package', packagePath);
