...
export async function writePkgTpl({moduleName, targetPlatform, targetArch, version, scopedPackagePath}){
    ...
    const packageJsonContentForModule = pkgTpl({githubOrganizationScope, moduleName, targetPlatform, targetArch, version, scopedPackagePath});
...
export async function copyReleaseBuild({module, scopedPackagePath}){
    if(module.name === 'desktop-trampoline' ){
        ...
        const [prebuild] = releaseContent.filter(entry => entry.isFile() && entry.name.endsWith('.node')).map(entry => entry.name);
...
export async function preparePackage({module, targetPlatform, targetArch}){
    const scopedPackagePath = path.join(ROOT_DIR, 'node_modules', githubOrganizationScope, `${module.name}-${targetPlatform}-${targetArch}`);
    ...
    await writePkgTpl({moduleName: module.name, targetPlatform, targetArch, scopedPackagePath, version: module.version});
...
export async function prepareParcelWatcherPrebuildsPackages(){
    ...
    const parcelWatchPrebuilds = parcelPackages
        .filter(entry => entry.isDirectory() && entry.name.startsWith('watcher-'))
        .map(entry => { 
            return { dir: path.join(parcelScopedPackagesBasePath,  entry.name), name: entry.name};
        });
    ...
    for(const {name, dir} of parcelWatchPrebuilds){
        ...
        await mkdir(path.join(ROOT_DIR, 'node_modules', '@hackolade'), {force: true, recursive: true})
        ...
        hackoladeScopedPackagesPaths.push(dir);      
    }
