...
export async function getElectronAbi() {
	...
	const electronConf = electronPkgInformation && electronPkgInformation.length >= 1 && electronPkgInformation[0];
...
async function canInspectBaseDirectory(baseDirectory) {
	try {
		...
		if (baseDirectory !== ROOT_DIR && !baseDirectory.endsWith('node_modules')) {
			log('[error] failed to access and read the content of base directory %o with %O', baseDirectory, err);
		}
...
async function listModulesInBaseDirectory(baseDirectory) {
	...
	return installedModules.filter(fsEntry => fsEntry.isDirectory()).map(fsEntry => fsEntry.name);
}
...
const ignoreDirectories = discoveredModuleDirectories => {
	...
	const modulesToIgnore = ['watcher', '@electron', 'nan'];

	return discoveredModuleDirectories
		.filter(dir => !directoriesThatAreNotModule.includes(dir))
		.filter(dir => !modulesToIgnore.includes(dir));
};

const keepOnlyNativeModules = allDiscoveredModules => allDiscoveredModules.filter(module => module.native);
...
async function getModulePkg({ moduleDirectory, parentPath }) {
	try {
		...
		if (err.code !== 'ENOENT' && !moduleDirectory.endsWith('node_modules')) {
			...
			log('%o - fail to get module packages in %o with %O', moduleDirectory, parentPath, err);
		}
		return [];
	}
}

async function gatherInstalledModuleInformation({ baseDirectory, moduleDirectory }) {
...
export async function discoverRegularNativeModules(baseDirectory) {
	try {
		...
		const modulesInBaseDirectory = await listModulesInBaseDirectory(baseDirectory);
		const sanitizedListOfInstalledModules = ignoreDirectories(modulesInBaseDirectory);
		...
		if (err.code !== 'ENOENT' && !baseDirectory.endsWith('node_modules')) {
			// handle potential missing nested node_modules
			log('failed discovering native modules from directory %o with %O ', baseDirectory, err);
		}
