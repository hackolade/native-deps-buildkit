...
export async function readPackageData(modulePath, moduleName) {
...
export async function listModulesInBaseDirectory(baseDirectory) {
	...
	const scopedModules = await Promise.all(scopes.map(async fsEntry => {
		const packages = await readdir(path.join(baseDirectory, fsEntry.name), {
			withFileTypes: true,
		});

		return packages.filter(fsEnt => fsEnt.isDirectory()).map(fsEnt => path.join(PROJECT_NODE_MODULES, fsEntry.name, fsEnt.name))
	}));

	const topLevelModules = installedModules.filter(fsEntry => fsEntry.isDirectory() && !fsEntry.name.startsWith('@')).map(fsEntry => path.join(PROJECT_NODE_MODULES, fsEntry.name));
