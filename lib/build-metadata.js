...
export function getModuleNameWithoutScope({ name, scope }) {
	if (scope && scope !== '') {
		return name.replaceAll(scopeReplacementRexp(scope), '');
	}
...
export function rescopeModuleBaseDir({ moduleBaseDir, scope, newScope }) {
	if (scope && scope !== '' && moduleBaseDir.includes(scope)) {
		return moduleBaseDir.replaceAll(scopeReplacementRexp(scope), `${newScope}/`);
	}

	const moduleBaseDirParts = moduleBaseDir.split('/');
	const module = moduleBaseDirParts.pop();

	return join('/', ...moduleBaseDirParts, newScope, module);
}
...
export async function getOfficialPrebuildsFromNpmOptionalDependencies({ scope, optionalDependencies = [], version }) {
	...
	return Promise.all(Object.entries(optionalDependencies)
...
export async function definePrebuildsAsNPMDependencies({prebuilds_as_npm_packages, name, version, scope, optionalDependencies}){
...
export async function getNativeModulesMetadata() {
	...
	const nativeModulesWithProjectPath = Object.entries(nativeModules).map(async ([moduleName, meta]) => {
		const [moduleBaseDir] = moduleInstalledPaths.filter(path => path.endsWith(`/${moduleName}`));
