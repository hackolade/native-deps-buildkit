...
export function rescopeModuleBaseDir({ moduleBaseDir, scope, newScope }) {
	if (scope && scope !== '' && moduleBaseDir.includes(scope)) {
		return moduleBaseDir.replaceAll(scopeReplacementRexp(scope), `${newScope}${sep}`).replaceAll('-napi', '').replaceAll('-glibc', '');
	}

	const moduleBaseDirParts = moduleBaseDir.split(sep);
	const module = moduleBaseDirParts.pop().replaceAll('-napi', '').replaceAll('-glibc', '');
...
export function sanitizeModulePackageJSON({
	...
	if (modulePkg?.dependencies && modulePkg?.dependencies['prebuild-install']) {
		delete modulePkg.dependencies['prebuild-install']; // no need now that we have the prebuilds as npm packages
	}
	if (modulePkg.dependencies &&  modulePkg?.dependencies['bindings']) {
		delete modulePkg.dependencies['bindings']; // only useful for build phase
	}
