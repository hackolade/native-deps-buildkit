...
export function sanitizeModulePackageJSON({
	...
	if (modulePkg?.dependencies['prebuild-install']) {
	...
	if (modulePkg?.dependencies['bindings']) {
	...
	if (modulePkg?.dependencies['node-addon-api']) {
		delete modulePkg.dependencies['node-addon-api']; // the module should be patched to avoid having to use this package and just require the expected prebuild as an npm dependency directly
	}
