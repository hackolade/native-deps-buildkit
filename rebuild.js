...
import {writePkgTpl} from '#lib/install.js';
...
for(const {module, targetPlatform, targetArch} of modulesToBuild){
    if(module.name !== 'couchbase'){
        ...
        const nativeModuleScopedPackage = path.join(ROOT_DIR, 'node_modules', '@hackolade', `${module.name}-${targetPlatform}-${targetArch}`);
        ...
        await rename(prebuildSrc, path.join(nativeModuleScopedPackage, 'prebuild.node'));
        ...
        always-auth=true`;
        await writeFile(path.join(nativeModuleScopedPackage, '.npmrc'), ghPackageNpmrc);
